...
function midpointDisplacement(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    for (let j = 0; j < arr.length; j++) {
      ...
      const iterationMax = waveHeight / Math.pow(2, i + 1)
...
function AnimatedWavePath({
  ...
  return (
    <>
      <path
        className='full-motion wave-mask'
        d={keyframes[0]}
      ...
      <path
        className='reduced-motion wave-mask'
        d={keyframes[0]}
        mask={maskId ? `url(#${maskId})` : undefined}
      />
    </>
...
function StaticWavePath({
  ...
  const data = createPathData(random, width, height, numDisplacements)
